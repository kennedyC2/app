...
// Create Product
const createTrending = async (data, callback) => {
    // Check Method
    switch (data.method) {
        case "options":
            callback(200, {}, "json");
            ...
            if (title && brand && dColor && sex && quantity && id && sold && price && colors && category && tags && sizes && misc) {
                ...
                if (file) {
                    ...
                    try {
                        ...
                        const sub_directory_1 = directory.collection("trending");
                        await sub_directory_1.insertOne(response);
                        const sub_directory_2 = directory.collection("products");
                        await sub_directory_2.updateOne({ _id: new ObjectId(id) }, { $addToSet: { misc: "trending" } });
